        ...
        <div className="px-5 py-4 border-b">
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0">
              <p className="font-medium text-sm text-foreground truncate">{user?.firstName} {user?.lastName}</p>
        ...
        {navItems.map((item) => (
          <NavLink key={item.path} to={item.path} end={item.path === "/dashboard"}
            className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all ${
              isActive(item.path) ? "nav-active" : "text-muted-foreground hover:bg-secondary hover:text-foreground"
            }`}>
