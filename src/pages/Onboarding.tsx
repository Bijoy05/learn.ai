    ...
    <div className="min-h-screen bg-background flex items-center justify-center">
      <AnimatePresence mode="wait">
            ...
            <div className="relative">
              {[...Array(20)].map((_, i) => (
                <motion.div
                  ...
                  style={{
                    background: i % 2 === 0 ? "hsl(252 90% 77%)" : "hsl(122 83% 80%)",
                    left: "50%",
                    top: "50%",
                  }}
        ...
        {phase === "welcome" && (
          ...
          >
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.5 }}>
              <Button size="lg" className="mt-8 rounded-xl px-10" onClick={() => setPhase("chat")}>
                Let's go
              </Button>
            </motion.div>
          </motion.div>
        ...
        {phase === "chat" && (
          <motion.div
            key="chat"
            className="w-full max-w-2xl mx-auto h-screen flex flex-col p-6"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
          >
            ...
            <div className="flex-1 overflow-y-auto space-y-4 pb-4">
              {messages.map((msg, i) => (
                ...
                >
                  <div className={`max-w-md rounded-2xl px-5 py-3 text-sm ${
                    msg.role === "ai" ? "bg-card border text-foreground" : "bg-primary text-primary-foreground"
                  }`}>
                    {msg.text}
                  </div>
                </motion.div>
                  ...
                  {q.type === "chips" && (
                    <div className="space-y-3">
                        ...
                        {q.options!.map((opt) => (
                          <button
                            ...
                            className={`px-4 py-2 rounded-xl text-sm border transition-all ${
                              selectedChips.includes(opt) ? "bg-accent text-accent-foreground border-accent" : "bg-card text-foreground hover:bg-secondary"
                            }`}
                          >
                            {opt}
                          </button>
                      ...
                      {selectedChips.length > 0 && (
                        <Button size="sm" className="rounded-xl" onClick={() => handleAnswer(selectedChips)}>
                          Continue
                        </Button>
                      )}
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {q.options!.map((opt) => (
                        <button
                          ...
                          className="p-4 rounded-xl border bg-card text-foreground text-sm text-left hover:bg-secondary hover:border-accent transition-all"
                        >
                          {opt}
                        </button>
                      ))}
